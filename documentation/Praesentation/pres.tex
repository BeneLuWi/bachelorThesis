...
$V_{0}:= \{x \in V\ |\ entweder\ x \in C_{B}\ oder\ x' \in C_{B} \}$ 
\end{lstlisting}
\only<1>{Zeilen 1-2: $n \cdot 2m$}
\only<2>{Zeilen 3-4: $\sqrt{n} \cdot m$ (LEDA:mcb\_matching, Hopcroft and Karp)}
\only<3>{Zeilen 5-6: $2n + k \cdot d$}
\only<4>{$n \cdot 2m + \sqrt{n} \cdot m + 2n + k \cdot d$}
\only<5>{$n \cdot 2m + \sqrt{n} \cdot m + 2n + k \cdot d \Rightarrow O(2mn)$}
\end{frame}
\begin{frame}{NT-Regel - Laufzeit}
\includegraphics[scale= .4]{analysisTrott_runtime.pdf} 
\end{frame}
